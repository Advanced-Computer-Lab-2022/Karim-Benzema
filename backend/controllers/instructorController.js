...
const upload = async (req,res) => {
    ...
    url+=`id=${videoId}&`;
    url+=`&part=contentDetails&key=AIzaSyDPt30Fz6GdNbKN1OfAHQZh61R6zqCL3xw`
    let time=0;
    const respondingg = await fetch(url).then((data)=> data.json())
    respondingg.items.map((item)=>{
        
       time= moment.duration(item.contentDetails.duration).asMinutes()

    })
    
    var data = mongoose.Types.ObjectId();
    data = await courses.findOne({_id:id}).select('_id')
    let coursing =await subtitle.find({course:data}).select('totalHoursSUB')
    let hours=0;
    for(let i=0;i<coursing.length;i++){
        if(coursing[i].totalHoursSUB!=undefined){
            let gr=Number(coursing[i].totalHoursSUB)
hours+=gr
console.log(gr)
    }
    }
 ...
 let totalHours= 5+time+hours+""
    
    let totalHoursSUB = 5+time+""
     
    const totalhoursubb = await subtitle.findOneAndUpdate({$and: [{course:data,number:number}]},
        {totalHoursSUB:totalHoursSUB},
        {new:true})
        const totalhourcourse = await courses.findOneAndUpdate({_id:data},
...
const moneyOwed= async (req, res) => {
    ...
    const data = await instructor.findOneAndUpdate({_id : instructorId},{earnings : totalEarnings},{new:true})
    if(!data){
        return res.status(404).json({error: "Not found"})
    }
    res.status(200).json(data)
  ...
  const getMyReviews= async(req,res)=>{
    const{id} = req.params
    const data= await reviews.find({instructorid:id})
    if(!data){
        return res.status(404).json({error: "instructor not found"})
    }
    res.status(200).json(data)
