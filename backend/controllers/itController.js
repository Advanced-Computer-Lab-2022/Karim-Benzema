...
const reviewss = require('../models/reviewsModel')
//add country
...
const getproblems = async (req,res) => {
    const data = await ITproblem.find({}).select('id Reporterid type report resolved')
    res.status(200).json(data)
}

const getproblems1 = async (req,res) => {
     const data1 = await INSTproblem.find({}).select('id Reporterid type report resolved')
    res.status(200).json(data1)
}

const getproblems2 = async (req,res) => {
     const data2 = await CTproblem.find({}).select('id Reporterid type Report resolved')
...
//}
    ...
    console.log(done)
            ...
            const deleteReview = async(req,res)=>{
                ...
                const review = await reviewss.findByIdAndDelete({_id:id})
                }
            
            const getMyReviews= async(req,res)=>{
                const{id} = req.params
                const data= await reviewss.find({userid:id})
                if(!data){
            ...
            const reviewInstructor = async (req,res) => {
                const {itid,id,reviews} = req.body
                const course = await courses.findOne({_id:id})
                const instID = course.instructor 
                const text = await reviewss.create({instructorid :instID, userid: itid, reviews : reviews})
