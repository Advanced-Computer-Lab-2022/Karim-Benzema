...
const searchawy = async (req,res) => {
    ...
    const data = await course.find({$or:[{title:{$regex:input.title}},{subject:{$regex:input.subject}},{instructorName:{$regex:input.instructorName}}]})
...
const mostPopular = async (req, res) => {
    try {
      // Find the top 3 most popular courses, sorted in descending order based on the registeredTrainees field
      const course = await courses.find().sort({ registeredTrainees: -1 }).limit(3);
